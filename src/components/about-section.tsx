...
export function AboutDrawer({ open, onClose, scrollToHelplines }: {
  open: boolean
  onClose: () => void
  scrollToHelplines?: boolean
}) {
  const { t } = useTranslation()
  ...
  useEffect(() => {
    if (open && scrollToHelplines) {
      const timer = setTimeout(() => {
        const el = document.getElementById('drawer-helplines')
        if (el) el.scrollIntoView({ behavior: 'smooth' })
      }, 100)
      return () => clearTimeout(timer)
    }
  }, [open, scrollToHelplines])
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative mx-auto mt-auto flex w-full max-w-[480px] max-h-[85vh] flex-col rounded-t-[20px] bg-card shadow-lg">
        ...
        <div className="overflow-y-auto px-5 py-5">
          ...
          <div className="mt-5 border-t border-border pt-4">
            <div id="drawer-helplines" className="space-y-0 text-[13px] leading-relaxed text-text-secondary">
              <p className="pb-3">
                This directory is provided &ldquo;as is&rdquo; for informational purposes only.
                It does not constitute medical advice, endorsement, or recommendation of any healthcare provider.
              </p>
              ...
              <p className="border-t border-border pt-3">
                ...
                Contact{' '}
                <a href="mailto:hello@rahulmax.com" className="underline">hello@rahulmax.com</a>{' '}
                for updates or removal requests.
              </p>
            </div>
          </div>
        </div>
